				...
				<div class="p-pic-list">
					...
					<div class="p-pic-wrap">
						<ul class="clearfix">
							...
							<!-- <li class="pic-item" v-for="(items,indexs) in photoInitList" v-tap="{methods:tapPhotosLast,index:indexs}"> -->
							<li class="pic-item" v-for="(items,indexs) in photoInitList">
								<div class="u-pic-wrap"><img :src="'data:image/jpeg;base64,'+items.photo"></div>
							</li>
							<li>
								<!-- 添加 -->
								<div class="p-pic-add-btn" v-tap="{methods:tapAddPhoto}" v-if="(photoList.length+photoInitList.length)<9">
									<!-- 新内容 -->
									<div class="upload-infor" v-if="(photoList.length+photoInitList.length)<2">
										<p>添加一张</p>
										<p>汗水照吧!</p>
									</div>
								</div>
...
<script>
	...
	export default{
		...
		mounted(){
			this.$nextTick(()=>{
				...
				if(typeof androidClient != "undefined" ){
					const resData=jsBridge.getWebViewPicture()
					this.photoInitList=[...this.photoInitList,resData]
				}

				// 初始化进入获取图片 ios
				jsBridge.getWebViewPicture((resData)=>{
					const newArr=[]
					resData.forEach((item,index)=>{
						this.$set(item,'photo',JSON.stringify(item.photo).replace(/\"/g, "").replace(/##R##/g,"\r").replace(/\\n/g,""))
						newArr.push(item)
				...
				// IOS发布
		        jsBridge.callback_action.uploadClassPublishPhoto=(resData)=>{
					this.tapPublish()
		        }
		...
		methods:{
			...
			tapPublish(){
				let uploadData='';
				const photoList=[...this.photoList,...this.photoInitList]
			...
			// 添加照片
			tapAddPhoto(){
				const photoSize=this.maxSize-this.photoList.length-this.photoInitList.length;
				...
				this.Ajax({
					...
					success:(res)=>{
						if(res.code==1){
							setTimeout(()=>{
								this.ajaxState=true
								jsBridge.uploadClassPublishState('/#/trainClass')
							},1500)
						}else{
							this.ajaxState=true
						}
